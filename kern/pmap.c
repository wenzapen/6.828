...
void print_page_free_list() {
	...
	for(int i=0; i<10; i++) {
		if(p) {
			cprintf("Checking page_free_list: %d\n",p-pages);
			p = p->pp_link;
		}
	}
...
mem_init_mp(void)
{
	...
	uintptr_t _va_start;
	physaddr_t _pa_start;
	for(int i=0; i<NCPU; i++) {
		_va_start = KSTACKTOP - i*(KSTKSIZE+KSTKGAP) - KSTKSIZE;
		_pa_start = PADDR(&percpu_kstacks[i]);
		boot_map_region(kern_pgdir,_va_start, KSTKSIZE, _pa_start, PTE_W);
	}	
...
page_alloc(int alloc_flags)
{
	...
	struct PageInfo *_page;
	if(!page_free_list) {
//		cprintf("out of page\n");
		return NULL;
	}
	_page = page_free_list;
	page_free_list = page_free_list->pp_link;
	_page->pp_link = NULL;
	if(alloc_flags & ALLOC_ZERO)
		memset(page2kva(_page), 0, PGSIZE);
//	cprintf("Allocated page id: %d \n", _page-pages);
	return _page;
}
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref || pp->pp_link)
		panic("Reference for this page is not NULL");
	if(pp > page_free_list) {
		pp->pp_link = page_free_list;
		page_free_list = pp;
	} else {
		next = head->pp_link;
		while( head > pp && next > pp ) {
			head = next;
			next = head->pp_link;
		}
		pp->pp_link = next;
...
//	cprintf("Freed page: %d(%x)\n",pp-pages,(uint32_t)pp);
}
...
{
	// Fill this function in
	pde_t * _pde; //va for page table
	pte_t * _pte; // va for page table entry
	int create_pt = 0;
	struct PageInfo *_page = NULL;
	if(!pgdir[PDX(va)]) { //if page table not exist, create one
		if(!create) {
			return NULL;
		}
		_page = page_alloc(ALLOC_ZERO);

		if(!_page) {
			return NULL;
		}
		_pde = (pde_t *)page2kva(_page);
		pgdir[PDX(va)] = page2pa(_page) | PTE_W |PTE_U| PTE_P; 
		_page->pp_ref++;
		create_pt = 1;
	//	cprintf("pgdir_walk:pgdir: %x page table: %d for va: %x not exist, created page: %d \n",pgdir,PDX(va),va,_page-pages);
	} else {
		_pde = KADDR(*&pgdir[PDX(va)] & 0xfffff000);
		_page = pa2page(*&pgdir[PDX(va)] & 0xfffff000);
	} 
	_pte = &_pde[PTX(va)];
/*	if(create_pt) 
		cprintf("pgdir_walk:PDX:%d PTX:%d pages: %d create pte: %x for va: %x\n",PDX(va),PTX(va),_page-pages,_pte,va);

	else 
		cprintf("pgdir_walk: PDX:%d PTX:%d pages: %d  pte: %x for va: %x\n",PDX(va),PTX(va),_page-pages,_pte, va);
*/
	return _pte;
}
...
{
	// Fill this function in
//	cprintf("pgdir %x va %x size %d pa %x perm %d\n",pgdir, va, size, pa, perm);
	pte_t * _pte;
	uint32_t _va, _pa, i;
	for(_va=(uint32_t)va,_pa=(uint32_t)pa,i=0;i<size/PGSIZE;_va+=PGSIZE,_pa+=PGSIZE,i++){
		_pte = pgdir_walk(pgdir, (const void *)_va, 1);
		*_pte = _pa | perm | PTE_P;
	} 
}
...
// if pp is in free list, remove it from free list
		if(pp == page_free_list) {
//			cprintf("I am the head\n");
			page_free_list = pp->pp_link;
			pp->pp_link = NULL;
		} else {
//			cprintf("I am not the head\n");
//			print_page_free_list();
			struct PageInfo *head = page_free_list;
			struct PageInfo *next = head->pp_link;
			while(next != pp && next != NULL) {
				head = next;
				next = head->pp_link;
			}
			head->pp_link = next->pp_link;
			pp->pp_link = NULL;
		}
	}
	pp->pp_ref++;
//	cprintf("pgdir :%x insert page: %d va:%x page.ref %d\n",pgdir,pp-pages,(uint32_t)va, pp->pp_ref);
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	pte_t *_pte;
	struct PageInfo * _page = NULL;
	_pte = pgdir_walk(pgdir, va, 0);
	if(!_pte) {
//		cprintf("page_lookup:pgdir: %x page table entry for va: %x not exist\n",pgdir, va);
		return NULL;
	}
	if(pte_store)
		*pte_store = _pte;
//	cprintf("lookup va:%x page: %d\n",(uint32_t)va, pa2page((uint32_t)(*_pte) & 0xfffff000)-pages);
	_page = ((uint32_t)(*_pte) & PTE_P) ? pa2page((uint32_t)(*_pte) & 0xfffff000) : NULL;
	return _page;
}
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	struct PageInfo *_page;
	_page = page_lookup(pgdir, va, 0);
	if(_page) {
		page_decref(_page);
		*(pgdir_walk(pgdir, va, 0)) = 0;
		tlb_invalidate(pgdir, va);
	}
//	cprintf("leaving page_remove\n");
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t _va = va_start;
	for(; _va<va_end; _va+=PGSIZE) {
        	pte_t * pt = (pte_t *)KADDR(PTE_ADDR(env->env_pgdir[PDX(_va)]));
        	if(!(pt[PTX(_va)] & (perm | PTE_P))) {
			if(_va < (uint32_t)va)
				user_mem_check_addr = (uintptr_t)va;
			else 
...
check_page_alloc(void)
{
	...
	for (pp = page_free_list, nfree = 0; pp; pp = pp->pp_link)
		++nfree;
	cprintf("Number of free pages: %d first free page: %d\n", nfree,page_free_list-pages);
...
//		cprintf("Check_ker_pgdir: pgdir: %x UPAGES: %x\n",pgdir, UPAGES);
//		cprintf("Check_ker_pgdir: PADDR(pages)+i: %x \n",PADDR(pages) + i);
		assert(check_va2pa(pgdir, UPAGES + i) == PADDR(pages) + i);
//		cprintf("Check_ker_pgdir: pages array: %d UPS:%x succeeded\n",i/PGSIZE,UPAGES+i);
	}
...
check_va2pa(pde_t *pgdir, uintptr_t va)
{
	...
	pgdir = &pgdir[PDX(va)];
//	cprintf("check_va2pa: va: %x pde addr: %x pde content: %x\n",va,pgdir,*pgdir);
	if (!(*pgdir & PTE_P))
		return ~0;
	p = (pte_t*) KADDR(PTE_ADDR(*pgdir));
//	cprintf("check_va2pa: va: %x pte addr: %x pte content: %x\n",va,p,*p);
	if (!(p[PTX(va)] & PTE_P))
		return ~0;
